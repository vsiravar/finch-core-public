...
binaries: rootfs lima-template lima
download: download.rootfs
lima: lima-exe
FINCH_IMAGE_LOCATION := $(FINCH_ROOTFS_LOCATION)
FINCH_IMAGE_DIGEST := $(FINCH_ROOTFS_DIGEST)
FEDORA_YAML := windows.fedora.yaml
else 
...
# TODO: PoC Windows specific. TODO make 7z/tar a variable 
...
lima-template: download
	...
	cp lima-template/$(FEDORA_YAML) $(OUTDIR)/lima-template/fedora.yaml
